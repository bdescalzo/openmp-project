...
double silhouette_simple(float elem[][NCAR], struct lista_grupos *listag, float cent[][NCAR], float *a){
    ...
    double aK[ngrupos],bK[ngrupos],maxK; // Terminos del indice de validacion
	double sk[ngrupos];
    double suma,S;
    S=0;
...
void analisis_enfermedades (struct lista_grupos *listag, float enf[][TENF], struct analisis *prob_enf)
{
    ...
    for (int i=0;i<TENF;i++){
        ...
        for(int j=0;j<ngrupos;j++){
            ...
            for (int k=0;k<listag[j].nelemg;k++){
                probs[k]=enf[listag[j].elemg[k]][i];
            }

            // Ordenamos las probabilidades de la enfermedad i en el grupo j
            for (int k = 0; k < listag[j].nelemg; k++) {
                for (int t = 0; t < listag[j].nelemg-1; t++) {
