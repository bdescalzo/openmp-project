...
int main (int argc, char *argv[]) {
	float   densidad[MAX_GRUPOS]; // densidad de cada cluster
	int     popul[MAXE]; // grupo de cada elemento
	float   cent[MAX_GRUPOS][NCAR]; // centroides
	int     i, j, nelem, grupo, num, nwrite, ind;
	int     fin = 0, num_ite = 0;
	...
	struct timespec  t11, t12, t17, t20, t21, t22, t23, t24;
	double texe, t_lec, t_clust, t_enf, t_escri, t_silh = 0, t_grup_cer = 0;

	if ((argc < 3)  || (argc > 4)) {
		fprintf(stderr, "[!] ERROR: %s bd_muestras bd_enfermedades [num_elem]\n", argv[0]);
		exit (-1);
	}
	...
	nwrite = fscanf (fd, "%d", &nelem);
	error_fscanf_escritura(fd, nwrite, argv[1]);
	if (argc == 4) nelem = atoi(argv[3]);	// 4. parametro: numero de elementos

	for (i=0; i<nelem; i++)
		for (j=0; j<NCAR; j++){
			nwrite = fscanf (fd, "%f", &(elem[i][j]));
			error_fscanf_escritura(fd, nwrite, argv[1]);
		}
	...
	for (i=0; i<nelem; i++) {
		for (j=0; j<TENF; j++){
			nwrite = fscanf(fd, "%f", &(enf[i][j]));
			error_fscanf_escritura(fd, nwrite, argv[2]);
		}
	}
	fclose (fd);
	...
	while(ngrupos<MAX_GRUPOS && convergencia_cont<1){
		...
		while ((fin == 0) && (num_ite < MAXIT)) {
			...
			clock_gettime (CLOCK_REALTIME, &t23);
			grupo_cercano (nelem, elem, cent, popul);
			clock_gettime (CLOCK_REALTIME, &t24);
			t_grup_cer += (t24.tv_sec-t23.tv_sec) + (t24.tv_nsec-t23.tv_nsec)/(double)1e9;
		...
		#pragma omp parallel for private(i, grupo, num) shared(popul, nelem, listag) schedule(runtime)
		for (i=0; i<nelem; i++){
			grupo = popul[i];
			#pragma omp critical
			{
		...
		clock_gettime (CLOCK_REALTIME, &t24);
		t_silh += (t24.tv_sec-t23.tv_sec) + (t24.tv_nsec-t23.tv_nsec)/(double)1e9;
	...
	if (fd == NULL) {
		fprintf (stderr, "[!] ERROR al abrir el fichero dbgen_out.s\n");
		exit (-1);
	}
	...
	fprintf (fd,">> Densidad de los clusters: a[i]\n\n");
	ind = 0;
	for (i=0; i<ngrupos/10; i++) {
		for (j=0; j<10; j++){
			fprintf (fd, "%9.2f", densidad[ind]);
			ind++;
		}
		fprintf (fd, "\n");
	}
	for(i=ind; i<ngrupos; i++) fprintf(fd, "%9.2f", densidad[i]);
	fprintf(fd, "\n");

	fprintf (fd,">> Analisis de enfermedades (medianas) en los grupos\n\n");
	for (i=0; i<TENF; i++)
	...
	t_escri = (t2.tv_sec-t22.tv_sec) + (t2.tv_nsec-t22.tv_nsec)/(double)1e9;
	...
	printf(">> Densidad de los clusters: a[i]\n\n");
	ind = 0;
	for (i=0; i<ngrupos/10; i++) {
		for (j=0; j<10; j++){
			printf("%9.2f", densidad[ind]);
			ind++;
		}
		printf("\n");
	}
	for(i=ind; i<ngrupos; i++) printf("%9.2f", densidad[i]);
	printf("\n");

	printf (">> Analisis de enfermedades en los grupos\n\n");
	for (i=0; i<TENF; i++)
		printf ("Enfermedad: %2d - mmax: %4.2f (grupo %2d) - mmin: %4.2f (grupo %2d)\n",
				i, prob_enf[i].mmax, prob_enf[i].gmax, prob_enf[i].mmin, prob_enf[i].gmin);
