...
void grupo_cercano (int nelem, float elem[][NCAR], float cent[][NCAR], int *popul)
{
    ...
    {
    #pragma omp for private(i, j, aInvestigar, min, centMenor) schedule(static)
    for (i=0;i<nelem;i++){
        min=-1;
        centMenor=0;
...
double silhouette_simple(float elem[][NCAR], struct lista_grupos *listag, float cent[][NCAR], float *a){
    ...
    for (i=0;i<ngrupos;i++){
		...
		// En caso contrario, calculamos la distancia acorde a la formula del CVI
            ...
            #pragma omp parallel for private(j, k) reduction(+:suma) schedule(dynamic, 4)
        ...
        suma = 0;
        #pragma omp parallel for private(j) reduction(+:suma) schedule(static)
        for(j=0;j<ngrupos;j++){
...
void analisis_enfermedades (struct lista_grupos *listag, float enf[][TENF], struct analisis *prob_enf)
{
    ...
    #pragma omp parallel for private(i, j, k, t) shared(listag, enf, ngrupos) schedule(static)
    for (i=0;i<TENF;i++){
        // Por cada grupo (j)
        for(j=0;j<ngrupos;j++){
